...
class Gene {
  ...
  Gene childGene(){
    if (mutationRate > random(0.0, 1.0)){
      ...
      return new Gene(predatorGene, preyGene, uncoGene);
    }
  }
  string showBinary(){
    String str = "";
  ...
  String description() {
    return '' + predatorGene + ' | ' + preyGene + ' | ' + uncoGene
  }
}
  ...
  void unco(){
    ...
    if(random(0,9)<=1){
      for (int i = 0; i < 2; i++) {
        ...
        Life res = Life.makeResource(positionx, positiony, resourceSize * 0.3, new Gene(gene.unco,0,0));
